...
// Les informations relative a une pi�ce.
struct PieceInfo {
	PieceInfo(){
		isSelect = false;
		tolZone = 0.0f;
	...
	// La liste des information du tableau a sauvegard�es.
	string serial;
	bool isSelect;
	double tolZone;
	string creationDate;
...
private:
	...
	// Les vecteur qui contienne les information n�cessaires a �valu� les indices dans le temps...
	vector<double> evolution_moyennes;
	vector<double> evolution_cp;
	vector<double> evolution_cpk;

	//contient la liste des test qui ont �chou�s...
	...
	// Donn�es des limites de contr�les des cartes I/MR
	double LCS_X;
	double LCI_X;
	double LC_X;
	double LCS_MR;
	...
	// Comme les valeurs �volue dans le temps il faut a un moment les fig� pour voir
	// comment �volue la capabilit� dans le temps.
	double LCX_fix;
	double LCS_X_fix;
	double LCI_X_fix;
	string date_LCX_fix;
	...
	// Evolution des indice qualit� piece par piece...
    void calculEvolution(unsigned int size);

	// Ce champs d�termine si l'analyse doit ce mettre automatiquement a jour
	bool isStatic_;

	// Vector temporaire qui garde en m�moire les valeurs.
...
public:
	...
	//accesseurs pour les param�tres de tests
	double getTest_K1();
	int getTest_K2();
	int getTest_K3();
	int getTest_K4();
	...
	//Accesseurs pour les �tats des tests
	...
	//la liste des tests �chou�s...
	QVector<int> getFailedTests();
	void setFailedTests(QVector<int>);

	//Mutateurs pour les param�tres de test
	...
	//Accesseur pour les cartes de contr�le I/MR
	double getLCS_X();
	double getLCI_X();
	double getLC_X();
	...
	//Matateurs pour les cartes de contr�le I/MR
    ...
    //tableau des �tats de chaque point des 2 courbes pour afficher les point d'erreur en rouge
	vector<Erreur> state_X;
	vector<Erreur> state_MR;

	//tableau contenant les donn�es � �tre analys� par l'analyseur (graphe Individual value)
    vector<double> donneesIndivitual;

	//tableau contenant les cote z de chaque donnee
	vector<double> coteZDonneeIndivitual;

	//tableau contenant les donn�es � �tre analys� par l'analyseur (graphe tolerance)
	...
	//m�thode pour trier les pi�ces par date..
	void sortPieceByDate();

	// Mise a jour des donn�e calcul� de l'analyse.
	void update();
	...
	// * Les donn�e relative au mod�le, num�ros d'op�ration etc, doivent avoir �t� initialis� correctement
