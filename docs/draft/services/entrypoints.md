...
| mcp | `golang/mcp/main.go` → `globular-mcp-server` | `/var/lib/globular/mcp/config.json` (or `~/.config/globular/mcp.json`) | HTTP JSON-RPC on `:10250` by default; stdio mode optional | Tools surface for AI/ops; POST `/mcp`, GET `/health`; SSE optional; session header `Mcp-Session-Id`. |
