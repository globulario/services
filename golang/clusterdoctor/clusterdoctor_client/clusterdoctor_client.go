...
type ClusterDoctorClient struct {
	cc *grpc.ClientConn
	c  clusterdoctorpb.ClusterDoctorServiceClient
...
func (c *ClusterDoctorClient) Reconnect() error {
	...
	for i := 0; i < 10; i++ {
		...
		if err == nil {
			c.c = clusterdoctorpb.NewClusterDoctorServiceClient(c.cc)
			return nil
		}
...
func (c *ClusterDoctorClient) GetClusterReport(ctx context.Context) (*clusterdoctorpb.ClusterReport, error) {
	return c.c.GetClusterReport(ctx, &clusterdoctorpb.ClusterReportRequest{})
}

func (c *ClusterDoctorClient) GetNodeReport(ctx context.Context, nodeID string) (*clusterdoctorpb.NodeReport, error) {
	return c.c.GetNodeReport(ctx, &clusterdoctorpb.NodeReportRequest{NodeId: nodeID})
}

func (c *ClusterDoctorClient) GetDriftReport(ctx context.Context, nodeID string) (*clusterdoctorpb.DriftReport, error) {
	return c.c.GetDriftReport(ctx, &clusterdoctorpb.DriftReportRequest{NodeId: nodeID})
}

func (c *ClusterDoctorClient) ExplainFinding(ctx context.Context, findingID string) (*clusterdoctorpb.FindingExplanation, error) {
	return c.c.ExplainFinding(ctx, &clusterdoctorpb.ExplainFindingRequest{FindingId: findingID})
}
