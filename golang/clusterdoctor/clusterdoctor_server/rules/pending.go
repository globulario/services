	...
	return []Finding{
		{
			...
			InvariantID: p.id,
			Severity:    clusterdoctorpb.Severity_SEVERITY_INFO,
			Category:    p.category,
			EntityRef:   "cluster",
			Summary:     p.summary,
			Evidence: []*clusterdoctorpb.Evidence{
				kvEvidence("clusterdoctor", "pending", map[string]string{
					"invariant_id":     p.id,
			...
			InvariantStatus: clusterdoctorpb.InvariantStatus_INVARIANT_PENDING,
		},
	}
}
