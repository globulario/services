...
func New(cfg CollectorConfig, cc clustercontrollerpb.ClusterControllerServiceClient) *Collector {
	return &Collector{
		cfg:              cfg,
		controllerClient: cc,
...
func (c *Collector) fetch(ctx context.Context) (*Snapshot, error) {
	...
	nodesResp, err := c.controllerClient.ListNodes(listCtx, &clustercontrollerpb.ListNodesRequest{})
	if err != nil {
		snap.addError("clustercontroller", "ListNodes", err)
	} else {
		snap.Nodes = nodesResp.GetNodes()
		snap.addSource("clustercontroller.ListNodes")
	}
	...
	healthResp, err := c.controllerClient.GetClusterHealthV1(healthCtx, &clustercontrollerpb.GetClusterHealthV1Request{})
	if err != nil {
		snap.addError("clustercontroller", "GetClusterHealthV1", err)
	} else {
		for _, nh := range healthResp.GetNodes() {
			snap.NodeHealths[nh.GetNodeId()] = nh
		}
		snap.addSource("clustercontroller.GetClusterHealthV1")
	}
...
func (c *Collector) fetchPerNode(ctx context.Context, snap *Snapshot) {
	...
	for _, node := range snap.Nodes {
		...
		if endpoint == "" {
			snap.addError("nodeagent@"+nodeID, "dial", fmt.Errorf("node %s has no agent_endpoint", nodeID))
			continue
		}
		...
		go func(nid, ep string) {
			...
			if err != nil {
				snap.addError("nodeagent@"+nid, "dial", err)
				return
			}
			...
			defer cancel()
			invResp, err := agentClient.GetInventory(invCtx, &nodeagentpb.GetInventoryRequest{})
			if err != nil {
				snap.addError("nodeagent@"+nid, "GetInventory", err)
			} else {
				snap.mu.Lock()
				snap.Inventories[nid] = invResp.GetInventory()
				snap.mu.Unlock()
				snap.addSource("nodeagent.GetInventory@" + nid)
			}

			// GetPlanStatusV1
			psCtx, cancel2 := context.WithTimeout(ctx, c.cfg.NodeTimeout)
			defer cancel2()
			psResp, err := agentClient.GetPlanStatusV1(psCtx, &nodeagentpb.GetPlanStatusV1Request{})
			if err != nil {
				snap.addError("nodeagent@"+nid, "GetPlanStatusV1", err)
			} else {
				snap.mu.Lock()
				snap.PlanStatuses[nid] = psResp.GetStatus()
				snap.mu.Unlock()
				snap.addSource("nodeagent.GetPlanStatusV1@" + nid)
			}

			// GetNodePlanV1 (from controller, using node id)
			planCtx, cancel3 := context.WithTimeout(ctx, c.cfg.NodeTimeout)
			defer cancel3()
			planResp, err := c.controllerClient.GetNodePlanV1(planCtx, &clustercontrollerpb.GetNodePlanV1Request{
				NodeId: nid,
			})
			if err != nil {
				snap.addError("clustercontroller@"+nid, "GetNodePlanV1", err)
			} else if planResp.GetPlan() != nil {
				snap.mu.Lock()
				snap.NodePlans[nid] = planResp.GetPlan()
				snap.mu.Unlock()
				snap.addSource("clustercontroller.GetNodePlanV1@" + nid)
			}
		}(nodeID, endpoint)
	}
...
func (c *Collector) agentClient(endpoint string) (nodeagentpb.NodeAgentServiceClient, error) {
	c.connMu.Lock()
	defer c.connMu.Unlock()

	if conn, ok := c.agentConns[endpoint]; ok {
		return nodeagentpb.NewNodeAgentServiceClient(conn), nil
	}
