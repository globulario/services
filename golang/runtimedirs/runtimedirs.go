// Package runtimedirs owns the single canonical-hyphen ↔ legacy-underscore
// alias model for service runtime directories under /var/lib/globular/.
//
// This is the ONE place the alias map lives. Two consumers share it:
//   - cluster-doctor (the DETECTOR) classifies layout drift and flags legacy
//     alias dirs as cleanup candidates rather than scary unknown drift.
//   - node-agent (the SELF-HEALER) canonicalizes legacy alias dirs on startup.
//
// Keeping the map here makes it impossible for the two to drift apart — there
// is no second handwritten list. See invariant
// service.runtime_dir_name_must_be_canonical and failure mode
// filesystem.service_runtime_dir_naming_drift.
package runtimedirs

import "strings"

// canonicalToLegacy maps a canonical service runtime dir (hyphenated package
// name) to its known legacy alias names. Only services with multiple historical
// names need an entry. Services whose only known name is the canonical (no
// aliases ever existed) do not appear here.
...
// legacyToCanonical is the reverse index built once at init. Built
// deterministically from canonicalToLegacy so the two maps cannot drift.
...
// CanonicalToLegacy returns a copy of the canonical→legacy-aliases model so
// callers (e.g. the node-agent startup sweep) can iterate every pair without
// being able to mutate the shared map. The slices are copied too.
func CanonicalToLegacy() map[string][]string {
	out := make(map[string][]string, len(canonicalToLegacy))
	for canon, aliases := range canonicalToLegacy {
