...
// loadReleaseIndex wraps bundlesync.LoadReleaseIndex. The shared
// implementation accepts the production BOM schema (v1/v2 packages array,
// AWARENESS_BUNDLE entry), the flat shape used by older tooling and tests,
// and the {"active": {...}} nested shape used by some dev tooling. Both this
// MCP caller and the globularcli install caller route through the same
// parser so a new release-index shape only has to be taught to one place.
