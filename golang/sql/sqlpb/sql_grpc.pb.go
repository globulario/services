...
type SqlServiceClient interface {
	...
	QueryContext(ctx context.Context, in *QueryContextRqst, opts ...grpc.CallOption) (grpc.ServerStreamingClient[QueryContextRsp], error)
...
func (c *sqlServiceClient) QueryContext(ctx context.Context, in *QueryContextRqst, opts ...grpc.CallOption) (grpc.ServerStreamingClient[QueryContextRsp], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &SqlService_ServiceDesc.Streams[0], SqlService_QueryContext_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[QueryContextRqst, QueryContextRsp]{ClientStream: stream}
...
type SqlService_QueryContextClient = grpc.ServerStreamingClient[QueryContextRsp]
...
type SqlServiceServer interface {
	...
	// Executes a query and returns zero or more rows, typically used for SQL SELECT.
	QueryContext(*QueryContextRqst, grpc.ServerStreamingServer[QueryContextRsp]) error
	// Executes an SQL statement like CREATE, INSERT, UPDATE, and DELETE.
	// Returns the number of affected rows and the last inserted ID, if applicable.
...
func (UnimplementedSqlServiceServer) QueryContext(*QueryContextRqst, grpc.ServerStreamingServer[QueryContextRsp]) error {
	return status.Error(codes.Unimplemented, "method QueryContext not implemented")
}
func (UnimplementedSqlServiceServer) ExecContext(context.Context, *ExecContextRqst) (*ExecContextRsp, error) {
	return nil, status.Error(codes.Unimplemented, "method ExecContext not implemented")
}
...
func _SqlService_QueryContext_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(SqlServiceServer).QueryContext(m, &grpc.GenericServerStream[QueryContextRqst, QueryContextRsp]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type SqlService_QueryContextServer = grpc.ServerStreamingServer[QueryContextRsp]
