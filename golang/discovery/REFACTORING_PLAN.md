# Discovery Service Refactoring Plan - Phase 1

## Goal
Apply proven Echo refactoring pattern to Discovery service to validate pattern generalization.

## Current State Analysis

**Files:**
- `server.go` (536 lines) - Everything: struct, getters/setters, main(), TLS, lifecycle
- `discovery.go` (310 lines) - RPC handlers (PublishService, PublishApplication)

**Problems:**
1. **God object**: 40+ fields mixing metadata, policy, TLS, runtime
2. **Unclear boundaries**: Discovery-specific vs Globular boilerplate unclear
...
6. **Complex permissions**: RBAC permissions mixed with initialization
...
├── handlers.go        # Business logic (PublishService, PublishApplication RPCs)
├── lifecycle.go       # Start/Stop hooks, readiness
├── server_test.go     # Unit tests
├── handlers_test.go   # Handler tests
├── lifecycle_test.go  # Lifecycle tests
└── discovery.go       # DEPRECATED, becomes handlers.go
```
...
#### handlers.go (renamed from discovery.go)
```go
// PublishService RPC handler - business logic
func (s *server) PublishService(ctx context.Context, req *discoverypb.PublishServiceRequest) (*discoverypb.PublishServiceResponse, error)

// PublishApplication RPC handler - business logic
func (s *server) PublishApplication(ctx context.Context, req *discoverypb.PublishApplicationRequest) (*discoverypb.PublishApplicationResponse, error)
...
- [ ] Add unit tests for PublishService() handler
- [ ] Add unit tests for PublishApplication() handler
...
- Commit frequently
- Follow exact Echo pattern

## Comparison with Echo Pattern
...
**Differences:**
- Discovery has RBAC permissions (complex map structures)
- Discovery has actual business logic (PublishService, PublishApplication)
- Discovery has dependencies on rbac.RbacService and resource.ResourceService

**Key Learning:**
If Discovery refactoring succeeds following Echo pattern exactly, the pattern is proven to generalize to services with real business logic.
...
- Repository service refactoring (PR5 - next after this)
