...
func TestReleaseAvailableWhenMinReplicasMatch(t *testing.T) {
	...
	updated := obj.(*cluster_controllerpb.ServiceRelease)
	if updated.Status.Phase != cluster_controllerpb.ReleasePhaseAvailable {
...
func TestReleaseDegradedWhenMismatchedButMinSatisfied(t *testing.T) {
	...
	obj, _, _ := srv.resources.Get(ctx, "ServiceRelease", rel.Meta.Name)
	updated := obj.(*cluster_controllerpb.ServiceRelease)
	if updated.Status.Phase != cluster_controllerpb.ReleasePhaseDegraded {
...
func TestReleaseFailedWhenMinNotSatisfied(t *testing.T) {
	...
	obj, _, _ := srv.resources.Get(ctx, "ServiceRelease", rel.Meta.Name)
	updated := obj.(*cluster_controllerpb.ServiceRelease)
	if updated.Status.Phase != cluster_controllerpb.ReleasePhaseFailed {
...
func TestDriftDoesNotDispatchWhenActivePlanLockHeld(t *testing.T) {
	...
	rel := testRelease(desiredHash, 0)
	rel.Status.Nodes = []*cluster_controllerpb.NodeReleaseStatus{{NodeID: "n1"}}
	if _, err := srv.resources.Apply(ctx, "ServiceRelease", rel); err != nil {
		t.Fatalf("apply release: %v", err)
	}
...
func testRelease(desiredHash string, maxUnavailable uint32) *cluster_controllerpb.ServiceRelease {
	return &cluster_controllerpb.ServiceRelease{
		Meta: &cluster_controllerpb.ObjectMeta{Name: "my-release", Generation: 1},
