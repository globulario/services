...
func (r *DNSReconciler) ConfigureExternalDNS(spec *cluster_controllerpb.ClusterNetworkSpec) error {
...
func (r *DNSReconciler) publishExternalDNS(ctx context.Context, spec *cluster_controllerpb.ClusterNetworkSpec) error {
...
func (r *DNSReconciler) isInMigration(spec *cluster_controllerpb.ClusterNetworkSpec) bool {
	if spec == nil || spec.DomainMigration == nil {
		return false
	}
	return spec.DomainMigration.State == cluster_controllerpb.DomainMigration_MIGRATION_IN_PROGRESS
}

// shouldCleanupOldDomain checks if grace period has expired and old domain should be cleaned up (PR9)
func (r *DNSReconciler) shouldCleanupOldDomain(spec *cluster_controllerpb.ClusterNetworkSpec) bool {
	if spec == nil || spec.DomainMigration == nil {
		return false
	}

	migration := spec.DomainMigration
	if migration.State != cluster_controllerpb.DomainMigration_MIGRATION_IN_PROGRESS {
		return false
	}
