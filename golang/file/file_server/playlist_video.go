...
func processVideos(srv *server, token string, dirs []string) {
	client, err := getMediaClient()
	if err != nil {
		slog.Error("media client failed", "err", err)
		return
	}
	...
	for _, d := range dirs {
		...
		if st, err := storage.Stat(ctx, path); err == nil {
			if st.IsDir() {
				for _, p := range filePerms {
					if strings.HasPrefix(p.Path, path) {
						_ = rbac_client_.DeleteResourcePermissions(token, p.Path)
						if err := rbac_client_.SetResourcePermissions(token, p.Path, p.ResourceType, p); err != nil {
							slog.Warn("perm update failed", "path", p.Path, "err", err)
						}
					}
				}
			} else if perm != nil {
				...
				perm.Path = path
				if err := rbac_client_.SetResourcePermissions(token, path, perm.ResourceType, perm); err != nil {
					slog.Warn("perm update failed", "path", path, "err", err)
				}
			}
		}
		if err := client.CreateVideoPreview(path, 360, 5); err != nil {
			slog.Warn("preview failed", "path", path, "err", err)
		}
		if err := client.CreateVideoTimeLine(path, 360, 5); err != nil {
			slog.Warn("timeline failed", "path", path, "err", err)
		}
		if err := client.GeneratePlaylist(path, token); err != nil {
			slog.Warn("playlist failed", "path", path, "err", err)
		}
	}
}
