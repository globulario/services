...
func (minioInfraProbeRequiredWhenInstalled) Evaluate(snap *collector.Snapshot, _ Config) []Finding {
	...
	for _, node := range snap.Nodes {
		...
		findings = append(findings, Finding{
			FindingID:   FindingID("minio.probe_required_when_installed", nid, ""),
			InvariantID: "minio.probe_required_when_installed",
			Severity:    sev,
			Category:    "minio",
			EntityRef:   nid,
			Summary:     fmt.Sprintf("Node %s has MinIO installed but produced no infra probe: %s", nid, reason),
			Evidence: []*cluster_doctorpb.Evidence{
				kvEvidence("node_agent", "GetInfraProbe", map[string]string{
					"node_id":             nid,
