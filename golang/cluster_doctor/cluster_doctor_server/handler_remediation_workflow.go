...
// StartRemediationWorkflow runs the remediate.doctor.finding workflow in
// process and returns a summary that mirrors the per-step run outputs.
// The workflow wraps ExecuteRemediation (does not bypass it).
//
// See services/golang/workflow/definitions/remediate.doctor.finding.yaml
// and services/docs/remediation_workflow.md.
func (s *ClusterDoctorServer) StartRemediationWorkflow(
	...
	run, err := s.RunRemediationWorkflow(ctx,
		req.GetFindingId(),
		req.GetStepIndex(),
	...
	if v, ok := run.Outputs["verification"].(map[string]any); ok {
