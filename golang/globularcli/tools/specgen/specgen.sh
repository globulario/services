...
EOF
  ...
  # NOTE: unit content below MUST stay in lockstep with the validated template
  # in golang/deploy/specgen.go (TestGenerateSpec_MatchesExisting pins this).
  # Single-sourcing the two templates is a tracked follow-up.
  tls_wait='ExecStartPre=/bin/sh -c '"'"'for i in $(seq 1 60); do [ -f /var/lib/globular/pki/issued/services/service.crt ] && exit 0; sleep 1; done; echo "TLS cert not ready after 60s"; ls -la /var/lib/globular/pki/issued/services/ 2>&1 || echo "cert dir missing"; exit 1'"'"''
