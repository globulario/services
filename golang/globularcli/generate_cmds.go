...
func init() {
	...
	generateServiceCmd.Flags().IntVar(&genPort, "port", 0, "default gRPC port (required; ports resolve from etcd at runtime)")
	...
	generateAllCmd.Flags().IntVar(&genPort, "port", 0, "default gRPC port (required; ports resolve from etcd at runtime)")
	...
	generateSpecCmd.Flags().IntVar(&genPort, "port", 0, "default gRPC port (required; ports resolve from etcd at runtime)")
