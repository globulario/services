...
func (srv *server) ResumeRun(ctx context.Context, clusterID, runID string, actorEndpoints map[string]string) error {
	...
	// ── 3. Load definition: etcd → local disk (bootstrap fallback) ───────
	// Mirror ExecuteWorkflow exactly so resume sees the same definition
	// as fresh dispatch. All workflow definitions live in etcd under
	// /globular/workflows/; local disk is a bootstrap-only fallback.
	var defYAML []byte
	...
	if len(defYAML) == 0 {
		for _, path := range []string{
			"/var/lib/globular/workflows/" + run.WorkflowName,
			"/usr/lib/globular/workflows/" + run.WorkflowName,
		} {
	...
	if len(defYAML) == 0 {
		return fmt.Errorf("workflow definition %q not found (etcd + local disk checked)", run.WorkflowName)
	}
