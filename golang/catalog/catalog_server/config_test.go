...
func TestDefaultConfigValidates(t *testing.T) {
	...
	if cfg.Name != "catalog.CatalogService" {
		t.Fatalf("Name = %q, want catalog.CatalogService", cfg.Name)
	}
...
func TestConfigFileRoundTrip(t *testing.T) {
	...
	cfg.Port = 1234
	cfg.Services["Persistence"] = map[string]any{"Address": "example.local:5555"}
	...
	if loaded.Services["Persistence"].(map[string]any)["Address"] != "example.local:5555" {
		t.Errorf("Services.Persistence.Address = %v, want example.local:5555", loaded.Services["Persistence"])
	}
}

func TestCloneDeepCopy(t *testing.T) {
	cfg := DefaultConfig()
	cfg.Keywords = []string{"Catalog", "Service"}
	cfg.Dependencies = []string{"dep1"}
	cfg.Services["Persistence"] = map[string]any{"Address": "addr1"}
	...
	clone.Services["Persistence"].(map[string]any)["Address"] = "addr2"
	...
	if cfg.Services["Persistence"].(map[string]any)["Address"] == "addr2" {
		t.Error("mutating clone.Services affected original")
	}
