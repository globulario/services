...
#   a. Removes the node from the cluster controller via `globular cluster nodes
#      remove` (cascades to envoy/xDS endpoint removal and MinIO pool eviction).
...
# Uses `globular cluster nodes remove` which cascades to: xDS endpoint removal,
# MinIO pool eviction, and node registry cleanup on the controller.
