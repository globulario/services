...
# Spec forests that are deliberately NOT package recipes and must not be scanned
# as though they were. packages/fixtures/ holds material for packages that were
# removed from the release set but whose evidence is retained on purpose — see
# the libnss-resolve removal, where the .deb and its scripts are kept precisely
# so the reason it was dangerous outlives the dependency. build.sh does not scan
# fixtures/, and neither should the authority check.
NON_RECIPE_SPEC_PREFIXES = ("packages/fixtures/",)


def is_services_generated(entry: dict) -> bool:
    """True when the services repo, not packages/, authors this package's spec.

    packages/registry.yaml remains the sole author of identity and kind, but a
    package declaring version_source: code has its spec GENERATED from the
    committed source in services (authzgen + the deploy spec generator) rather
    than hand-authored under packages/metadata/. Requiring a hand-authored
    metadata spec for those is the pre-2026-08 contract; packages dropped them
    deliberately ("generated, not hand-authored here"). A generated package MAY
    still carry metadata (mcp keeps one as an extra_template) and it is validated
    normally when present — only the requirement is lifted.
    """
    return str(entry.get("version_source", "")).strip() == "code"
...
def check_metadata_specs(
    ...
    for name in sorted(registry_by_name):
        if name not in spec_by_name:
            if is_services_generated(registry_by_name[name]):
                continue
...
def check_duplicate_recipe_roots(errors: list[str], services_root: Path, packages_root: Path, installer_root: Path) -> None:
    ...
    for base in search_roots:
        for path in base.rglob("*.yaml"):
            ...
            if path_rel.startswith(NON_RECIPE_SPEC_PREFIXES):
                continue
...
def main() -> int:
    ...
    for entry in registry_entries:
        if entry.get("kind") in RUNTIME_KINDS and entry["name"] not in canonical_specs:
            if is_services_generated(entry):
                continue
